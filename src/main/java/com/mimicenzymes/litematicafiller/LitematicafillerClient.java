...
public class LitematicafillerClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        // 注册自定义网络包
        PayloadTypeRegistry.playC2S().register(RemoteInventoryPacket.ID, RemoteInventoryPacket.CODEC);
        PayloadTypeRegistry.playS2C().register(RemoteInventoryResponsePacket.ID, RemoteInventoryResponsePacket.CODEC);

        ClientPlayNetworking.registerGlobalReceiver(RemoteInventoryResponsePacket.ID, (payload, context) -> {
            context.client().execute(() -> RemoteFetcher.onResponse(payload.success()));
        });
