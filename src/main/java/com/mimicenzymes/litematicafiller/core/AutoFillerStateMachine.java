...
public class AutoFillerStateMachine {
    ...
    public enum Phase {
        ...
        FILLING,
        RETURNING,
        REMOTE_FETCHING,   // 等待远程取物响应
        REMOTE_FETCH      // 从远程容器取物
    }
    ...
    private ItemStack remoteFetchTargetItem;
    private MemoryLocation remoteFetchLocation;
    ...
    private void requestNbtUpdate(MinecraftClient client, BlockPos pos) {
        ...
        //#else
        if (!client.isInSingleplayer() && client.player != null && client.player.getPermissions().hasPermission(DefaultPermissions.GAMEMASTERS)) {
        //#endif
            try {
    ...
    public void addTask(BlockPos pos, Map<Integer, ItemStack> requiredItems) {
        ...
        if (!isCrafter && (requiredItems == null || requiredItems.isEmpty())) {
            return;
        }
        // 确保 requiredItems 不为 null
        Map<Integer, ItemStack> safeRequired = requiredItems == null ? new HashMap<>() : requiredItems;
    ...
    private boolean checkMaterialsAndPrepare(MinecraftClient client) {
        ...
        if (!hasAtLeastOneMaterial) {
            if (Configs.ENABLE_REMOTE_FETCH.getBooleanValue()) {
                // 取第一个缺失的物品
                ItemStack firstMissing = currentTask.missingItems.values().iterator().next();
                RemoteFetcher.fetch(firstMissing);
                currentPhase = Phase.REMOTE_FETCHING;
                return false;  // 重要：等待远程取物
            } else {
    ...
    public void tick(MinecraftClient client) {
        ...
        if (actionWaitTicks <= 0 && actionQueue.isEmpty() && currentTask != null && !yieldTick) {
            ...
            switch (currentPhase) {
                ...
                case IDLE:
                    break;
                case REMOTE_FETCHING:
//                    System.out.println("[LitematicaFiller-DEBUG] tick: 处于 REMOTE_FETCHING 状态");
                    if (!RemoteFetcher.isWaiting()) {
//                        System.out.println("[LitematicaFiller-DEBUG] RemoteFetcher.isWaiting = false");
                        if (RemoteFetcher.isSuccess()) {
                    ...
                    // 针对空潜影盒：检测是否已打开远程容器 GUI
                    if (client.currentScreen instanceof HandledScreen<?> &&
                            !(client.player.currentScreenHandler instanceof net.minecraft.screen.PlayerScreenHandler)) {
//                        System.out.println("[LitematicaFiller-DEBUG] 检测到外部容器 GUI 已打开");
                        remoteFetchTargetItem = RemoteFetcher.getTargetItem();
                        remoteFetchLocation = RemoteFetcher.getTargetLocation();
                        if (remoteFetchTargetItem != null && remoteFetchLocation != null) {
//                            System.out.println("[LitematicaFiller-DEBUG] 切换到 REMOTE_FETCH 状态");
                            currentPhase = Phase.REMOTE_FETCH;
                            break;
                        }
                    }

                    // 背包检测（适用于 litematica-printer 标准取物）
                    ItemStack target = RemoteFetcher.getTargetItem();
                    if (target != null && hasItemInInventory(client, target)) {
//                        System.out.println("[LitematicaFiller-DEBUG] 背包中已检测到目标物品，取物成功");
                        RemoteFetcher.onFetchSuccess();
                        currentPhase = Phase.IDLE;
                        break;
                    }

                    // 超时检查（5秒）
                    if (System.currentTimeMillis() - RemoteFetcher.getRequestTime() > 5000) {
//                        System.out.println("[LitematicaFiller-DEBUG] 远程取物超时，放弃");
                        RemoteFetcher.onFetchFailed(target);
                        reset();
                        break;
                    }
                    break;
                case REMOTE_FETCH:
                    ...
                    doRemoteFetch(client);
                    break;
            }
        }
    }
    private boolean hasItemInInventory(MinecraftClient client, ItemStack target) {
    ...
    private void doRemoteFetch(MinecraftClient client) {
//        System.out.println("[LitematicaFiller-DEBUG] doRemoteFetch 开始执行");
        ...
        for (int i = 0; i < handler.slots.size() - 36; i++) {
            ...
            if (ItemMatcher.isSameItem(stack, remoteFetchTargetItem)) {
                containerSlot = i;
//                System.out.println("[LitematicaFiller-DEBUG] 在容器槽位 " + i + " 找到目标物品");
                break;
            }
        }
        if (containerSlot == -1) {
//            System.out.println("[LitematicaFiller-DEBUG] 容器中未找到目标物品");
            client.player.closeHandledScreen();
            RemoteFetcher.onFetchFailed(remoteFetchTargetItem);
            reset();
            return;
        }
        ...
        if (emptySlot == -1) {
            sendFeedback(client, "litematica_container_filler.message.inventory_full_cannot_extract", true);
            client.player.closeHandledScreen();
            RemoteFetcher.onFetchFailed(remoteFetchTargetItem);
            reset();
            return;
        ...
        // 更新缺失物品：移除已取回的物品（注意：只移除一个，因为一次只取一个潜影盒）
        currentTask.missingItems.values().removeIf(item -> ItemMatcher.isSameItem(item, remoteFetchTargetItem));

        // 通知 RemoteFetcher 成功并重置
        RemoteFetcher.onFetchSuccess();

        // 重置远程取物相关变量
        remoteFetchTargetItem = null;
        remoteFetchLocation = null;

        // 将状态设为 IDLE，并清空 actionQueue，让主循环重新进入材料检查
        currentPhase = Phase.IDLE;
        actionQueue.clear();

        // 注意：不要在这里添加额外的 actionQueue，主循环会自动处理
    }
    private void resetRemoteFetchState() {
        remoteFetchTargetItem = null;
        remoteFetchLocation = null;
        currentPhase = Phase.IDLE; // 或者回到上一阶段，但由队列控制
    }
    ...
    public void onRemoteFetchSuccess(ItemStack item, MemoryLocation location) {
        if (currentPhase == Phase.REMOTE_FETCHING) {
            remoteFetchTargetItem = item;
            remoteFetchLocation = location;
            currentPhase = Phase.REMOTE_FETCH;
        }
    }

    public void onRemoteFetchFailed(ItemStack item) {
//        System.out.println("[LitematicaFiller-DEBUG] Running onRemoteFetchFailed");
        MinecraftClient client = MinecraftClient.getInstance();
        sendFeedback(client,Text.translatable(
                        "litematica_container_filler.message.material_shortage",item.getName().getString()).getString(),
                true);
        reset();
    }
}